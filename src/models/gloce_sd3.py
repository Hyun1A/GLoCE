...
class SimpleSelectorOutProp(nn.Module):
    def __init__(self, gate_rank, d_model, dropout=0.25, n_concepts=1, is_last_layer=False, res_rank=16):
        super().__init__()        
        ...
        self.select_residue = ParamModule((n_concepts, d_model, res_rank))
        self.select_residue.weight.data = self.select_residue.weight.data * 0.0
    ...
    def forward(self, x, mask=None):
        ## x: (B,T,D)
        # SD3 gate (per-token): normalize -> remove surrogate-basis projection -> renormalize ->
        # squared projection onto discriminative basis -> sigmoid.
        B, T, D = x.size()

        x_norm = x / x.norm(2, dim=2, keepdim=True)
        Vh_res = self.select_residue.weight  # (N,D,H)
        Vh_gate = self.select_weight.weight  # (N,D,r)

        x_ = torch.einsum("btd,ndh->bnth", x_norm, Vh_res)
        residue = x_norm.unsqueeze(1) - torch.einsum("bnth,ndh->bntd", x_, Vh_res)
        residue = residue / residue.norm(dim=3, keepdim=True)

        cont = (torch.einsum("nds,bntd->bnts", Vh_gate, residue)**2).sum(dim=-1)  # (B,N,T)

        select_scale = torch.sigmoid( self.imp_slope.unsqueeze(0).unsqueeze(-1)*( \
                    cont - self.imp_center.unsqueeze(0).unsqueeze(-1)) )  # (B,N,T)

        select_scale, select_idx = select_scale.max(dim=1, keepdim=True)  # (B,1,T)
...
class GLoCELayerOutProp(nn.Module):
    ...
    def __init__(
        ...
        elif org_module.__class__.__name__ == "JointTransformerBlock":
            out_dim = org_module.attn.out_dim
        else:
            raise ValueError(f"unsupported module type: {org_module.__class__.__name__}")
    ...
    def apply_to(self):
        ...
        if self.org_module.__class__.__name__ == "JointTransformerBlock":
            self.org_module.forward = self.forward_sd3_block
        else:
            self.org_module.forward = self.forward
        del self.org_module

    def forward_sd3_block(self, hidden_states, encoder_hidden_states, temb):
        encoder_hidden_states, hidden_states = self.org_forward(hidden_states, encoder_hidden_states, temb)
        if self.find_name == "sd3_block_context":
            if encoder_hidden_states is not None:
                encoder_hidden_states = self.forward_gloce(encoder_hidden_states)
        else:
            hidden_states = self.forward_gloce(hidden_states)
        return (encoder_hidden_states, hidden_states)

    def forward(self, x):
        x = self.org_forward(x)
        return self.forward_gloce(x)

    def forward_gloce(self, x):
        # x.shape: (B, T, D)
        self.t_counter +=1
