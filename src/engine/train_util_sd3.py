...
from diffusers import UNet2DConditionModel, SchedulerMixin, DiffusionPipeline, StableDiffusion3Pipeline
...
def encode_prompts(
    ...
    (
        ...
        prompts, prompts_2, prompts_3, ## prompt 
        negative_prompt=['' for p in prompts],
        device=pipe.device, do_classifier_free_guidance=False,
...
@torch.no_grad()
def embedding2img_sd3(embs, pooled_embs, pipe, neg_prompt='', save_path='./', save_name=None, device='cuda:0', guidance_scale = 7.0, image_size=512,
                      sample_step=28, max_step=28, num_samples=1, from_case=0, to_case=None,seed=0, target="", uncond_embeddings=None, end_timestep=-1):
    
    
    batch_size = len(embs)
    
    neg_emb, neg_pooled = encode_prompts(pipe, [neg_prompt] * batch_size)
    generator = torch.cuda.manual_seed(seed)
    
    prompt_embeds = torch.cat([neg_emb, embs], dim=0)
    pooled_prompt_embeds = torch.cat([neg_pooled, pooled_embs], dim=0)
    ...
    timesteps, num_inference_steps = retrieve_timesteps(pipe.scheduler, max_step, device)
    
    for i, t in enumerate(tqdm(timesteps[:sample_step])):
        # expand the latents if we are doing classifier free guidance
        latent_model_input = torch.cat([latents] * 2) if guidance_scale <= 0.0 else latents
        # broadcast to batch dimension in a way that's compatible with ONNX/Core ML
    ...
    temp = (latents / pipe.vae.config.scaling_factor) + pipe.vae.config.shift_factor

    image = pipe.vae.decode(temp, return_dict=False)[0]
    pipe.image_processor.postprocess(image.detach().cpu())[0].save(f'{save_path}')
