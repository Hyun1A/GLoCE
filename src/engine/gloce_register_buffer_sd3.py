    ...
    def get_org_hook(find_m_name, name):
        def forward_hook(module, input, output):
            if module.__class__.__name__ == 'JointTransformerBlock':
                output = output[0] if find_m_name == "sd3_block_context" else output[1]
            ...
            if (t > st_timestep) and (t <= end_timestep):
                ...
                data = output[B//2:]
                if ("to_add_out" in name) or (find_m_name == "sd3_block_context"):
                    data_flat = data[:, 1:(1+n_avail_tokens)].reshape((B//2) * n_avail_tokens, -1)
                elif ("attn2.to_k" in name) or ("attn2.to_v" in name):
...
@torch.no_grad()
def register_sum_buffer_avg_spatial_rel(args, module_name_list_all, org_modules_all, registered_buffer, hooks, \
                    st_timestep, end_timestep, n_avail_tokens, **kwargs):
    ...
    def get_org_hook(find_m_name, name):
        def forward_hook(module, input, output):
            ...
            Vh_sur_dict = kwargs["Vh_sur_dict"]
            ...
            if (t > st_timestep) and (t <= end_timestep):
                ...
                if ("to_add_out" in name) or (find_m_name == "sd3_block_context"):
                    data_flat = data[:, 1:(1+n_avail_tokens)].reshape((B//2) * n_avail_tokens, -1)
                elif ("attn2.to_k" in name) or ("attn2.to_k" in name):
                    data_flat = data[1:(1+n_avail_tokens)].view((B//2) * n_avail_tokens, -1)
                else:
                    data_flat = data.view((B//2)*T, -1)
                    width, height = int(data.size(1)**0.5), int(data.size(1)**0.5)
                    avg_depth = 5
                    data_spatial = [data.transpose(-1,-2).reshape(B//2, D, width, height)]
                    data_flatten = [] #[data_flat]
                    for idx in range(avg_depth-1):
                        data_spatial.append( F.avg_pool2d(data_spatial[-1], kernel_size=2, stride=2) )
                        if data_spatial[-1].shape[-1] > 8 :
                            continue                        
                        data_flatten.append(data_spatial[-1].view(B//2,D, -1).transpose(-1,-2))
                        b_,t_,d = data_flatten[-1].size()
                        data_flatten[-1] = data_flatten[-1].view(b_*t_,-1)
                ...
                Vh = Vh_sur_dict[find_m_name][name]
                data_flat_norm_res = data_flat_norm - ( (data_flat_norm @ Vh.T) @ Vh ) # TxD

                ###### Only consider the direction of residual ######
                data_flat_norm_res = data_flat_norm_res / data_flat_norm_res.norm(dim=1, keepdim=True)
            
                if registered_buffer[find_m_name][name]["data"] is None:
                    registered_buffer[find_m_name][name]["data"] = data_flat.T @ data_flat
                    registered_buffer[find_m_name][name]["data_norm"] = data_flat_norm_res.T @ data_flat_norm_res
    ...
    for find_module_name, module_name_list, org_modules in zip(args.find_module_name, module_name_list_all, org_modules_all):
        ...
        for n in module_name_list:
            registered_buffer[find_module_name][n] = {"t": 0, "data": None, "data_norm": None, "n_forward": 0, "n_sum_per_forward": 0}
            hook = org_modules[n].register_forward_hook(get_org_hook(find_module_name, n))
            hooks.append(hook)
...
@torch.no_grad()
def register_norm_buffer_avg_spatial_rel_stat_minmax_stack_v2(args, module_name_list_all, org_modules_all, registered_buffer, hooks, \
                    st_timestep, end_timestep, n_avail_tokens, **kwargs):
    ...
    def get_org_hook(find_m_name, name):
        def forward_hook(module, input, output):
            ...
            Vh_dict = kwargs["Vh_dict"]
            Vh_sur_dict = kwargs["Vh_sur_dict"]
            gate_rank = kwargs["gate_rank"]
            ...
            if (t > st_timestep) and (t <= end_timestep):
                ...
                if ("to_add_out" in name) or (find_m_name == "sd3_block_context"):
                    ...
                    avg_depth = 5
                    data_spatial = [data.transpose(-1,-2).reshape(B//2, D, width, height)]
                    # per-token stats (consistent with the per-token gate at inference)
                    data_flat = data.reshape((B//2)*T, -1)
                ...
                data = (Vh_dict[find_m_name][name]@data_flat_norm_res.T)**2 # DxT 
                ...
                if registered_buffer[find_m_name][name]["data"] is None:
                    ...
                    val_max, ind_max = data[:gate_rank].sum(dim=0).max(dim=0)  # D --> 1
